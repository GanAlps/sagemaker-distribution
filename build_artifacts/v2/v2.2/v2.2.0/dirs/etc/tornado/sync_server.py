...
class SyncPredictionHandler(tornado.web.RequestHandler):
    def initialize(self):
        self.response_msg = "Sync server created."
        try:
    ...
    def post(self):
        ...
        try:
            if self.customInference is not None :
                response = self.customInference.handle(data_str)
    ...
    def get(self):
        try:
            if self.customInference is not None :
                response = self.customInference.handle("Where is Seattle?")
                self.response_msg += ", custom inference response recorded"
            else :
                response = "Custom Inference Spec Not Present"
                self.response_msg += ", Custom Inference Spec Not Present"
...
def make_app():
    return tornado.web.Application([
        (r"/invocations", SyncPredictionHandler),
        (r"/ping", PingHandler),
    ])
...
async def main():
    app = make_app()
    app.listen(port)
    print(f"Starting Sync server on http://localhost:{port}")
    await asyncio.Event().wait()

if __name__ == "__main__":
    asyncio.run(main())
    print(f"Started Sync server on http://localhost:{port}")